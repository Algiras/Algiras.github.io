...
const MortgageCalculator: React.FC = () => {
  ...
  const [inputsStorage, setInputs] = useLocalStorage<MortgageInput>(
    'mortgage-calculator-inputs',
    defaultInputs
  );
  const [monthlyIncomeStorage, setMonthlyIncome] = useLocalStorage<number>(
    'mortgage-calculator-monthly-income',
    8000
  );
  ...
  const results = useMemo(() => {
    ...
    for (let month = 1; month <= totalPayments; month++) {
      ...
      let principalPayment =
        monthlyPrincipalInterest - interestPayment + extraPayment;
  ...
  const chartData = useMemo(() => {
    ...
    for (let year = 1; year <= Math.ceil(schedule.length / 12); year++) {
      ...
      yearlyData.push({
        ...
        cumulativePrincipal: Math.round(
          yearPayments[yearPayments.length - 1]?.cumulativePrincipal || 0
        ),
        cumulativeInterest: Math.round(
          yearPayments[yearPayments.length - 1]?.cumulativeInterest || 0
        ),
      });
    }
    ...
    return [
      {
        ...
        color: '#8884d8',
        percentage:
          (results.monthlyPrincipalInterest / results.totalMonthlyPayment) *
          100,
      },
      {
        name: 'Property Tax',
        ...
        color: '#82ca9d',
        percentage:
          (results.monthlyPropertyTax / results.totalMonthlyPayment) * 100,
      },
      {
        name: 'Home Insurance',
        ...
        color: '#ffc658',
        percentage:
          (results.monthlyInsurance / results.totalMonthlyPayment) * 100,
      },
        ...
        ? [
            {
              ...
              color: '#ff7300',
              percentage:
                (results.monthlyPMI / results.totalMonthlyPayment) * 100,
            },
          ]
        : []),
        ...
        ? [
            {
              ...
              color: '#8dd1e1',
              percentage:
                (results.monthlyHOA / results.totalMonthlyPayment) * 100,
            },
          ]
        : []),
    ];
            ...
            <Stack gap="md">
              ...
              <Group grow>
                <NumberInput
                  ...
                  onChange={value =>
                    updateDownPayment(Number(value) || 0, false)
                  }
                  min={0}
                  step={1000}
                  thousandSeparator=","
                ...
                <NumberInput
                  ...
                  onChange={value =>
                    updateDownPayment(Number(value) || 0, true)
                  }
                  min={0}
                  max={100}
                  step={0.5}
                  ...
                  >
                    <Stack gap="xs">
                      <Group justify="space-between">
                        ...
                        >
                          {results.affordabilityAnalysis.debtToIncomeRatio.toFixed(
                            1
                          )}
                          %
                        </Text>
        ...
        <Card shadow="sm" padding="lg" radius="md" withBorder>
          <Tabs defaultValue="overview" className="w-full">
            <Tabs.List>
              ...
              <Tabs.Tab
                value="breakdown"
                leftSection={<PieChart style={{ width: 16, height: 16 }} />}
              >
                Payment Breakdown
              </Tabs.Tab>
              <Tabs.Tab
                  ...
                  <Group justify="space-between" align="center">
                    <Stack gap="xs">
                      <Text fw={600}>
                        {results.affordabilityAnalysis.isAffordable
                          ? '✅ Affordable'
                          : '⚠️ May Not Be Affordable'}
                      </Text>
                      <Text size="sm" c="dimmed">
                        Based on debt-to-income ratio of{' '}
                        {results.affordabilityAnalysis.debtToIncomeRatio.toFixed(
                          1
                        )}
                        %
                ...
                <Group grow>
                  ...
                  <Card withBorder p="md">
                    <Stack gap="xs">
                      ...
                      <Text size="lg" fw={600}>
                        $
                        {results.affordabilityAnalysis.maxAffordablePayment.toFixed(
                          0
                        )}
                      </Text>
                    </Stack>
